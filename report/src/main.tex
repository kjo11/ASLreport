\documentclass[noabstract,doubleheaders,onlytitle,titlepage,oneside,openright,11pt]{epflthesis}
% \documentclass[doubleheaders,los,lof,lot,titlepage,oneside,openright,11pt]{epflthesis}
\includeonly{chapters/basics}

\input{packages}
\input{definitions}
\input{glossary}
\makeglossaries

\title{Visual Terrain Estimation for Legged Robots}
\author{Frederike Dümbgen}
\advisor{Alireza Karimi (EPFL)\\
Michael Blösch (ETHZ) \\
Philipp Krüsi (ETHZ) \\
Dominik Schindler (ETHZ) }
\submitdate{29 July 2016}
\school{Ecole polytechnique fédérale de Lausanne}
\dept{Mechanical Engineering}

\schoolii{Eidgenössische Technische Hochschule Zürich: Autonomous Systems Lab}
\deptii{}

\logo{images/eth_logo.pdf}
\city{Zurich}

\printlos{%
  \printglossary[type=latin]
}


\begin{document}

% \maketitle
% \tableofcontents
% \chapter*{Symbols}
% \printglossaries

\include{chapters/introduction}

\include{chapters/basics}

\include{chapters/methods-overview}
\include{chapters/methods-mapping}
\include{chapters/methods-localization}
\include{chapters/methods-outlook}

\include{chapters/results-datasets}
\include{chapters/results-mapping}
\include{chapters/results-localization}

\include{chapters/conclusion}


% \begin{appendices}
  % \include{appendix/parameters}
  % \include{appendix/planedataset}
% \end{appendices}

%\nocite{*} %Show everything in bibliography.
% \bibliographystyle{siam}
% \bibliography{bibliography}

\end{document}
